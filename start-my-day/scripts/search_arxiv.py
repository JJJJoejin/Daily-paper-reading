...
Used by the start-my-day skill to search for trending, popular, and high-quality papers from the past month and year
"""
...
def title_to_note_filename(title: str) -> str:
    ...
    ensuring wikilink paths generated by start-my-day correctly point to files created by paper-analyze.
    """
...
# Default category-to-keyword mapping (used when no user-defined keywords in config)
...
# ---------------------------------------------------------------------------
# Scoring constants -- edit weights here only
# ---------------------------------------------------------------------------

# Maximum raw score per dimension (normalization baseline)
SCORE_MAX = 3.0

# Relevance score: bonus for keyword matches in title / abstract
RELEVANCE_TITLE_KEYWORD_BOOST = 0.5
RELEVANCE_SUMMARY_KEYWORD_BOOST = 0.3
RELEVANCE_CATEGORY_MATCH_BOOST = 1.0

# Recency thresholds (days) -> corresponding scores
RECENCY_THRESHOLDS = [
    (30, 3.0),
    (90, 2.0),
    (180, 1.0),
...
# Popularity: influential citation count normalized to 0-SCORE_MAX
# Meaning: reaching this citation count is considered full score
POPULARITY_INFLUENTIAL_CITATION_FULL_SCORE = 100

# Composite recommendation score weights (regular papers)
WEIGHTS_NORMAL = {
    'relevance': 0.40,
    'recency': 0.20,
    'popularity': 0.30,
...
# Composite recommendation score weights (high-impact papers: increase popularity, decrease recency)
WEIGHTS_HOT = {
    'relevance': 0.35,
    'recency': 0.10,
    'popularity': 0.45,
...
# Semantic Scholar rate limit wait time (seconds)
S2_RATE_LIMIT_WAIT = 30
S2_CATEGORY_REQUEST_INTERVAL = 3

# Semantic Scholar API Key (optional, read from config file)
S2_API_KEY = None


def load_research_config(config_path: str) -> Dict:
    """
    Load research interests configuration from a YAML file
    ...
    try:
        ...
        # Read Semantic Scholar API Key (if configured)
        global S2_API_KEY
...
def calculate_date_windows(target_date: Optional[datetime] = None) -> Tuple[datetime, datetime, datetime, datetime]:
    """
    Calculate two time windows: last 30 days and past year (excluding last 30 days)

    Args:
        target_date: Base date; if None, uses the current date

    Returns:
        (window_30d_start, window_30d_end, window_1y_start, window_1y_end)
        - window_30d_start: 30-day window start date
        - window_30d_end: 30-day window end date (i.e. target_date)
        - window_1y_start: One-year window start date
        - window_1y_end: One-year window end date (i.e. 31 days ago)
    """
    if target_date is None:
        target_date = datetime.now()
    
    # Last 30 days window: [target_date - 30 days, target_date]
    window_30d_start = target_date - timedelta(days=30)
    window_30d_end = target_date
    
    # Past year window (excluding last 30 days): [target_date - 365 days, target_date - 31 days]
...
def search_arxiv_by_date_range(
    ...
    """
    Search for papers within a specified date range using the arXiv API
...
def search_semantic_scholar_hot_papers(
    ...
    """
    Search for high-impact papers within a specified time range using the Semantic Scholar API

    Args:
        query: Search keywords
        start_date: Start date
        end_date: End date
        top_k: Return top K high-impact papers
        max_retries: Maximum number of retries

    Returns:
        List of papers sorted by influential citation count
    """
    # Build date range (Semantic Scholar uses YYYY-MM-DD:YYYY-MM-DD format)
    ...
    params = {
        ...
        "limit": 100,  # First fetch top 100 most relevant papers
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Local secondary filtering and sorting
            valid_papers = []
            for p in papers:
                # Filter out invalid entries without title or abstract
                ...
                p["hot_score"] = inf_cit  # Use influential citation count as hot score
                
                valid_papers.append(p)
            
            # Sort by influential citation count in descending order
            ...
            if attempt < max_retries - 1:
                # For 429 errors, use a longer wait time
                if is_rate_limit:
...
def search_hot_papers_from_categories(
    ...
    Returns:
        Merged list of high-impact papers
    """
    all_hot_papers = []
    seen_arxiv_ids = set()

    # Extract user-defined search keywords from config (more precise)
    user_queries = []
    if config:
        domains = config.get('research_domains', {})
        for domain_name, domain_config in domains.items():
            keywords = domain_config.get('keywords', [])
            # Combine the first 3 keywords of each domain into a query
            if keywords:
                query = ' '.join(keywords[:3])
                user_queries.append(query)

    # If no user keywords, fall back to category keywords
    if not user_queries:
    ...
    for query in unique_queries:
        ...
        for p in papers:
            # Safely extract ArXiv ID from externalIds dictionary
            arxiv_id = p.get("externalIds", {}).get("ArXiv") if p.get("externalIds") else None
            
            # Write to arxiv_id field uniformly, for global deduplication in Step 3
            p["arxiv_id"] = arxiv_id
            ...
            if arxiv_id and arxiv_id not in seen_arxiv_ids:
                ...
                # Also keep papers without arXiv ID (may be from other sources)
                all_hot_papers.append(p)
...
def parse_arxiv_xml(xml_content: str) -> List[Dict]:
    ...
    Returns:
        List of papers, each containing ID, title, authors, abstract, etc.
    """
    papers = []
...
def calculate_relevance_score(
    ...
    """
    Calculate relevance score of a paper to research interests

    Args:
        paper: Paper information
        domains: Research domain configuration
        excluded_keywords: Keywords to exclude

    Returns:
        (relevance score, matched domain, list of matched keywords)
    """
...
def calculate_quality_score(summary: str) -> float:
    """
    Infer quality score from abstract

    Uses fine-grained indicators: strong innovation words are weighted higher than weak ones,
    and quantitative results and comparative experiments also receive bonus points.
...
def calculate_recommendation_score(
    ...
    Weights are defined in module-level constants WEIGHTS_NORMAL / WEIGHTS_HOT.
    For high-impact papers (from Semantic Scholar), WEIGHTS_HOT increases popularity weight.

    Args:
        relevance_score: Relevance score (0-SCORE_MAX)
        recency_score: Recency score (0-SCORE_MAX)
        popularity_score: Popularity score (0-SCORE_MAX)
        quality_score: Quality score (0-SCORE_MAX)
        is_hot_paper: Whether this is a high-impact paper
...
def filter_and_score_papers(
    ...
    Args:
        ...
        target_date: Target date (used for recency calculation)
        is_hot_paper_batch: Whether this is a high-impact paper batch
    ...
    for paper in papers:
        ...
        if 'published_date' in paper:
            ...
            # For Semantic Scholar papers, use publicationDate
        ...
        if is_hot_paper_batch:
            # High-impact papers: use influentialCitationCount
            ...
            # Regular papers (no citation data): assign intermediate popularity based on recency
            # Papers from the last 7 days may have higher "potential hotness"
            if 'published_date' in paper and paper['published_date']:
                ...
                if days_old <= 7:
                    popularity = 2.0  # Very new papers have potential hotness
                elif days_old <= 14:
                    popularity = 1.5
                elif days_old <= 30:
                    popularity = 1.0
                ...
                popularity = 0.5  # Conservative value when no date info available
...
def main():
    ...
    if not args.config:
        logger.error("Config file path not specified. Please set it via the --config argument or the OBSIDIAN_VAULT_PATH environment variable.")
        return 1
    ...
    # ========== Step 1: Search recent papers from the last 30 days (arXiv) ==========
    logger.info("=" * 70)
    ...
    # ========== Step 2: Search high-impact papers from the past year (Semantic Scholar) ==========
    ...
    # Deduplicate (prefer arXiv ID, then normalized title)
    seen_ids = set()
    seen_titles = set()
    unique_papers = []
    for p in all_scored_papers:
        ...
        if arxiv_id:
            ...
            # For papers without arXiv ID, deduplicate by title (normalize: lowercase + remove punctuation)
            title = p.get('title', '')
    ...
    # Add note_filename for each paper, consistent with generate_note.py filename rules
    # So start-my-day generated wikilinks can directly use this field without inference
